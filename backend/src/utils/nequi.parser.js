...
// Normalizar banco (función existente)
if (result.data.banco) {
  result.data.banco = normalizeBanco(result.data.banco);
}

// 🔥 NUEVA HEURÍSTICA: si el banco es "lar" y el texto contiene "bancolombia", corregir
if (result.data.banco === 'lar' && /bancolombia/i.test(text)) {
  result.data.banco = 'Bancolombia';
}
